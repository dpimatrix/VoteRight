...
export async function POST(request: Request) {
  ...
  // Separate forms on the page (general/stripe/check/donation_tiers) all
  ...
  // (Used to include a 4th, "authorizenet" -- removed with the rest of
  // Authorize.Net support, 2026-09-05.)
  const section = String(form.get("section") ?? "");

  if (section === "general") {
    // Blank here is never "leave unchanged" -- there's no valid state
    // where the fee is simply unset after a save was attempted, so an
    // empty/invalid submission is a real error, not silently ignored. Real
    // bug found live 2026-08-19: this used to accept blank the same
    // permissive way the key fields do, which let a save silently no-op
    // with zero visible error -- the fee stayed NULL for a full round of
    // "is it configured yet?" troubleshooting before the gap was found.
    ...
    await updatePaymentSettings({ feeCents });
