...
export default function CampaignScreen() {
  ...
  const EXT_STATUS_LABEL: Record<string, string> = {
    not_started: d.ext_status_not_started,
    gathering_signatures: d.ext_status_gathering,
    submitted: d.ext_status_submitted,
    certified: d.ext_status_certified,
    not_applicable: d.ext_status_na,
  };
  ...
  const load = useCallback(async () => {
    try {
      ...
      setError(d.campaign_load_error);
    }
  }, [id, d.campaign_load_error]);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={[styles.card, { backgroundColor: colors.backgroundElement }]}>
          ...
          {c.signature_requirement_note && (
            <ThemedText type="small" themeColor="textSecondary">
              {tf(d.signature_requirement_note, { note: c.signature_requirement_note })}
            </ThemedText>
          )}
        </View>
        ...
        <View style={[styles.card, { backgroundColor: colors.backgroundElement }]}>
          <ThemedText type="small" themeColor="textSecondary">
            {tf(d.supporters_count, { n: c.support_count, date: c.date })}
          </ThemedText>
          {c.mechanism_type === 'charter_amendment_petition' && (
            <ThemedText type="small" themeColor="textSecondary">
              {tf(d.petition_status_note, { status: EXT_STATUS_LABEL[c.external_petition_status] ?? c.external_petition_status })}
            </ThemedText>
          )}
          {c.supported ? (
            <ThemedText type="small">{d.already_supported}</ThemedText>
            ...
            <Pressable disabled={busy} onPress={support} style={[styles.actionBtn, { backgroundColor: colors.evidence }]}>
              <ThemedText type="smallBold">{d.support_campaign_btn}</ThemedText>
            </Pressable>
          ) : !verified ? (
            <Pressable onPress={() => router.push('/verify')} style={[styles.actionBtn, { backgroundColor: colors.evidence }]}>
              <ThemedText type="smallBold">{d.verify_to_support}</ThemedText>
            </Pressable>
          ) : null}
          <ThemedText type="small" themeColor="textSecondary">
            {d.support_attrib_note}
          </ThemedText>
        </View>
