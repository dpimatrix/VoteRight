    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      <View style={styles.content}>
        ...
        {/* Identity backup/recovery (2026-08-24) -- closes a real gap the
            ...
            priorities, debate history, and payment_verified status.
            Mirrors web's KeySettings.tsx (migration 088), ported to what's
            actually available on this SDK -- see signing.ts's and
            backup.tsx's own header comments for the full design. */}
        <ThemedText type="smallBold">{d.key_settings_h}</ThemedText>
        <View style={styles.group}>
          {/* '/backup' as Href: expo-router's typed-routes cache (.expo/types)
              is generated by the Metro dev server and hadn't picked up this
              new screen file yet at typecheck time -- regenerates itself
              automatically the next time `expo start` runs; the route
              itself is real. */}
          <Pressable onPress={() => router.push('/backup' as Href)} style={[styles.row, { backgroundColor: colors.backgroundElement }]}>
            <ThemedText type="small">{d.key_settings_h}</ThemedText>
          </Pressable>
        </View>
