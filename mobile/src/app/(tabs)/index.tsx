...
interface StackedOffice {
  ...
  // See jurisdictions.ts's StackedOffice (migration 081) -- whether
  // term_start is verified as the CURRENT term's start, not just inferred
  // from source. Defaults false (untrusted); only President/VP are
  // currently marked precise. Gate on this before trusting a computed
  // next-election year, same as web.
        ...
        {data?.jurisdictions.map((j) => {
          ...
          return (
            <View key={j.id} style={styles.section}>
              ...
              {rows.map((o) => {
                // Same real bug fix as web (2026-08-14, widened same day --
                // see jurisdictions.ts's StackedOffice for the full story):
                // an untracked non-municipal seat used to always read as
                // "Not yet tracked" even for seats with multi-year terms
                // not up this cycle. term_length > 2 skips House (always up
                // next cycle regardless of data); term_start_precise gates
                // everything else, defaulting to untrusted -- only
                // President/VP are currently marked precise.
