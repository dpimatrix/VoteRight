...
|---|---|---|---|
| Roll-call votes → scored positions | `voting_records` → `politician_positions`/`position_codings` | `db/ingest/votes.mjs` (Montgomery Socrata), `db/ingest/congress-votes.mjs` (U.S. House), `db/ingest/senate-votes.mjs` (U.S. Senate, both built 2026-09-22) | Montgomery County Council + the full U.S. Congress (House + Senate). **All 50 state legislatures and every other DMV-pilot council (PG/Fairfax/Arlington/DC) still have zero scored votes.** Raw votes only, everywhere — none of these are yet coded onto scoring axes (`politician_positions`); that's a separate, deliberate next step, see §3/D4. |
