...
|---|---|---|---|
| Roll-call votes → scored positions | `voting_records` → `politician_positions`/`position_codings` | `db/ingest/votes.mjs` (Montgomery Socrata), `db/ingest/congress-votes.mjs` (U.S. House), `db/ingest/senate-votes.mjs` (U.S. Senate), `db/ingest/state-legislature-votes.mjs` (state legislatures, all built 2026-09-22) | Montgomery County Council + the full U.S. Congress + **Maryland and Virginia state legislatures (first pass — 48 more states not yet run, deliberately, see the script's own header for why: MD alone has 26,430 historical bills in OpenStates, so this needs real per-state verification before a blind `--states=all` run, matching the exact same caution `openstates-legislature.mjs`'s own roster ingester already applied)**. Every other DMV-pilot council (PG/Fairfax/Arlington/DC) still has zero votes of any kind. Raw votes only, everywhere — none of these are yet coded onto scoring axes (`politician_positions`); that's a separate, deliberate next step, see §3/D4. |
