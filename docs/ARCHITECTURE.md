...
- **Storage: local VPS disk (`../debate-media/` alongside `app/`), not S3.** The original "S3-compatible object store" plan predates the move off Vercel/Neon (DEPLOY.md) — everything else already lives on the owner's own VPS disk, so this does too, consistent with the project's "earn your complexity" posture (§12).
