...
- **Security posture for running ffmpeg against untrusted uploads** (added 2026-08-15, prompted by the obvious question: an upload pipeline that feeds attacker-controlled bytes into a media parser is real attack surface, not a hypothetical one):
  ...
  - **What this does NOT close**: ffmpeg itself has a real CVE history in its many format demuxers/decoders, and this pipeline runs whatever format ffmpeg's auto-detection picks, unsandboxed, as the same `voteright` OS user as the rest of the app — no seccomp, no container isolation, no separate low-privilege user. That's the deepest residual risk here and it's genuinely how most video-hosting pipelines work (accepting diverse real-world upload formats requires a broad decoder surface), not something a quick fix closes; mitigated only by staying on a current ffmpeg build and `voteright` having no root/sudo on the box (bounds the blast radius, doesn't eliminate it). Real sandboxing (a dedicated low-priv user or container for the transcode step) is a legitimate follow-up, not yet built.
