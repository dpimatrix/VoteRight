...
**Not yet built, explicitly deferred**: real per-admin identity for the `reconciled_by` field — the whole admin console is currently one shared TOTP login, not per-person accounts (owner has since asked for role-based admin access; tracked in `docs/BACKLOG.md`, not yet designed as of this note). **Not yet tested against live credentials** — the Stripe and Authorize.Net integrations (`app/src/lib/paymentGateways/`) are written against each vendor's documented API shape but have not been run against a real (even sandbox) account; verify end-to-end before this reaches real users.
