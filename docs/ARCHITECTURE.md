...
- **Early debate closure has two independent paths, not one** (migrations 093 and 094, 2026-08-24) — a participant supermajority vote (`call_the_question_votes`) AND an admin's own force-close judgment (`thread_reports` + `forceCloseThread()`, required written `closed_reason`, shown publicly on the closed thread). The vote-based path was briefly removed outright (093) after a real live-testing finding — a supermajority-of-active-participants trigger with no floor on group size let a single tester's own vote reach 100% of a 1-participant thread, cutting the fixed `closes_at` window short before latecomers had a real chance to weigh in — then restored the same day (094) with two floors that close the actual gap without losing the underlying capability: a genuinely, broadly settled debate should still be able to advance early. See Section 12's "Calling the question" entry for the full writeup.
...
CREATE TABLE forum_threads (
    ...
    -- Early closure has TWO independent paths (migrations 093 and 094,
    -- 2026-08-24) -- see Section 12's "Calling the question" entry for the
    -- full writeup. closed_early/closed_early_at are "did this close before
    -- its natural closes_at date" regardless of which path did it.
    ...
    -- the two floors that actually close the original gap: the vote isn't
    -- even offered until BOTH are met, not just before it can succeed
...
-- member abuse reports on a thread (migration 093) -- the trigger for an
-- admin's force-close decision; one open report per (thread, user)
...
-- Notifications (migration 094) -- in-app + push + email, see Section 12's
-- "Calling the question" entry for the full design.
...
-- a user identity. thread_reports rows get the same private-signal treatment on a privacy
-- deletion request (privacy.ts's executeDeletion()).
...
**"Calling the question" was built, removed, then restored with real floors — all in one day (2026-08-24).** RONR's "previous question" motion solves a synchronous-meeting problem — a room, a clock, competing agenda items — that doesn't transfer to an async platform where no thread competes with another for meeting time. In its first form, `call_the_question_votes` let *any* active participant in a `forum_thread` (anyone who'd posted an argument, or cast 3+ agreement votes) vote to end debate immediately once `close_early_threshold_pct` (66.7%, matching RONR's default) of active participants agreed — and with a thread's `active` count as low as 1 (an early tester's single vote on their own single argument), that supermajority was trivially reachable by one person, cutting the fixed `closes_at` window short before latecomers had any real chance to weigh in. That undermined the one guarantee the window exists to give: a predictable, equal amount of time for everyone. Confirmed live 2026-08-24 testing a single-participant thread. Migration 093 removed the mechanism outright rather than patch it, and added a completely separate early-closure path instead: `thread_reports` lets a member flag a thread as abusive (thread-level, not per-argument — that failure mode is a thread devolving into spam/harassment across many posts, not one bad post, which `moderate()`'s existing per-argument review already covers); reports surface in `/admin/moderation`'s "Reported debate threads" queue; an admin who agrees action is warranted calls `forceCloseThread()`, which requires a written reason (shown publicly on the closed thread) and advances the proposal to `referendum` ahead of `closes_at`.

**The owner's follow-up, later the same day, drew the distinction sharper**: a supermajority vote being *exploitable* by a tiny group isn't the same as the underlying capability being illegitimate — a debate that's genuinely, broadly settled (real turnout, a real supermajority, no single actor able to trigger it alone) advancing early is a real benefit RONR is actually right about, not just a manipulation vector. The flaw was narrower than "the vote mechanism" — it was the complete absence of a floor on how few participants could exercise it. Migration 094 restored `call_the_question_votes` alongside two new columns on `forum_threads` that gate whether the vote is even *offered* at all, not just whether it can succeed:

- `call_the_question_min_active` — a real supermajority needs a real group, not 1-of-1.
- `call_the_question_min_open_hours` — even a fast, legitimate supermajority can't close a thread before someone who checks the app every few days has had a real window to see it — the actual harm from the original design, addressed directly rather than through participant count alone.

Both floors are pilot-scale defaults (3 active participants, 72 hours), same "real column, disclosed and auditable, tunable per jurisdiction" posture `close_early_threshold_pct` and `second_threshold` already have. `ctqVote()` (debates.ts) checks both before accepting a vote or letting it close the thread; `debateDetail()` computes `floorsMet` so the UI on both platforms doesn't even show the mechanism until it's genuinely available, distinct from `eligible` (is *this specific user* an active participant able to cast the vote).

**The two early-closure paths now coexist, deliberately, not redundantly**: calling-the-question handles "debate is genuinely settled, let's not make everyone wait" (a participant-driven signal, gated by real floors); reports + force-close handle "this has nothing to do with consensus, it's abuse" (a moderator-driven judgment, for a failure mode no vote threshold could ever fix — spam and harassment don't get better with a bigger quorum). `thread_reports` is watched by the same anomaly-detection velocity pipeline `call_the_question_votes` is (§9) — a report button is exactly as brigade-able as a vote button, just requiring a person's judgment before anything actually closes rather than an automatic percentage trigger.

**Notifications (migration 094)** close the loop on both paths: `notifications` (in-app, rendered client-side from `type` + the joined proposal/thread — never a pre-rendered string, since this app is bilingual throughout and a message baked in at write time would defeat that), `push_tokens` (Expo's free push service, no vendor decision — the app already runs on Expo), and an opt-in `users.notification_email`/`notification_email_verified_at` (owner's vendor choice: Resend; verified via a signed link before anything is ever sent to it, same HMAC-signed-payload shape `adminAuth.ts`'s session tokens already use). Two events trigger a notification: a thread closing (either path, immediate, fired inline from `ctqVote()`/`forceCloseThread()`) and a thread newly crossing both CTQ floors (`ctq_eligible`, detected by the periodic `close-and-notify-threads.mjs` background job, not any single write path, since it's a function of elapsed time as much as participant count). That same job also closes a real pre-existing gap found while building this: **nothing anywhere in the app previously transitioned a thread's status when its `closes_at` date simply passed** — the only two paths that ever closed a thread were a CTQ vote succeeding and an admin's force-close, so an unreported, un-called thread stayed `'open'`, arguments/votes still live, indefinitely past its stated 14-day window. **Known limitation, stated plainly**: push/email content is composed server-side with no per-user language preference to read (unlike every other server-rendered string in this app), so both render English-only for now; the in-app notification list is unaffected, rendered client-side in the reader's own current language like everything else.
...
- **Verification-tiered vote weight.** `unverified` users can read everything and draft priorities privately, but cannot second, argue, or vote until `address_verified` at minimum. **Superseded 2026-08-19** — debate participation (proposing, seconding, arguing, calling the question) now requires the top tier specifically, `payment_verified`, not just `address_verified`; referenda/mandates and accountability-campaign support are unchanged, still gated on `address_verified` alone. See §9.2.
- **Anomaly detection on the ingestion side of `seconds`, `referendum_ballot_tokens` issuance, and `thread_reports`**: velocity checks (many actions from one IP/device cluster in a short window), device fingerprint correlation, and geographic plausibility against `residence_jurisdiction_id`. `thread_reports` replaced `call_the_question_votes` in this watched-action slot when calling-the-question was removed (migration 093, 2026-08-24, Section 12) — reporting a thread is a smaller-N target than a full referendum (a thread might have only a few dozen "active participants"), the same "cheaper to brigade" shape CTQ brigading was, just aimed at getting a moderator to force-close a thread instead of an automatic vote closing it.
  - **Velocity + geographic checks implemented 2026-08-15** (`app/src/lib/anomalyDetection.ts`, migration 084) — confirmed live first that this bullet had been purely aspirational: zero layered defense existed behind the self-attested `address_verified` check, and the only "anomaly" system actually built (`user_key_events`) protects against a stolen *signing key*, a different problem entirely. What's built now covers exactly the three actions this bullet names, plus `address_verification` itself (the root action a Sybil attacker performs first): every occurrence is logged (`action_context_log`) for velocity counting, and a flag lands in a human review queue (`/admin/anomalies`) — never auto-blocks — when either too many distinct users share a context in a 30-minute window, or a request geolocates outside the US (coarse on purpose; IP geolocation isn't accurate enough for state-level comparison to be low-noise). **Device fingerprint correlation is NOT built** — the third leg this bullet names would mean either a paid third-party SDK (its own vendor-cost/privacy tradeoff, similar in shape to the `govt_id_verified` question below) or a much weaker in-house heuristic; deferred rather than built weakly. This narrows but does not close the structural gap in §13 item 9 below — it's a visibility layer over today's self-attested tier, not a replacement for a stronger one.
...
**Implemented 2026-08-14** (local text/audio/video upload + playback; YouTube-linked arguments above are still design-only, not built): concrete decisions made where the above was still TBD —

- **Storage: local VPS disk (`app/debate-media/`), not S3.** The original "S3-compatible object store" plan predates the move off Vercel/Neon (DEPLOY.md) — everything else already lives on the owner's own VPS disk, so this does too, consistent with the project's "earn your complexity" posture (§12). Lives under `app/`, not as a repo-root sibling — a `..`-traversing path confused Next's build-time file tracer into sweeping unrelated compiled routes into every route's trace (harmless for this non-standalone deployment, but avoided anyway).
- **Not served from `app/public/` like politician photos, on purpose.** Photos are always-public government portraits; a debate argument can be `pending` or `removed`, and `moderation_status` already hides pending/removed *text* arguments from everyone but their author (`debates.ts`). A static `public/` file has no such gate and would leak that same content forever via its raw URL regardless of what the UI shows. Media is instead served through `GET /api/debates/media/[id]`, which re-checks `moderation_status` (approved → public; pending/removed → author or an authenticated moderator only, 404 otherwise) on every request, and supports HTTP Range requests (206 Partial Content) — required for reliable `<video>` playback and seeking on Safari/iOS in particular.
- **Format: always transcoded to H.264 + AAC in an MP4 (video) / M4A (audio) container**, regardless of source format, via a self-installed static ffmpeg build (`app/src/lib/media.ts`) — capped at 1280×720 and CRF 26. Chosen over more space-efficient codecs (H.265/VP9/AV1) specifically because VoteRight has a real native mobile app, not just a web wrapper, and H.264 has no playback-compatibility risk anywhere; the space saving comes from the resolution/CRF caps, not from codec choice.
- **The 3-minute cap is enforced twice**: client-side (soft — a `<video>`/`<audio>` `loadedmetadata` check, for fast feedback before an upload even starts) and server-side via `ffprobe` reading the actual uploaded file's real duration, checked *before* transcoding starts so an oversized upload fails fast rather than burning CPU on a file that's getting rejected anyway. Only the server check is authoritative.
- **`transcript_text` is deliberately NOT generated in this pass** — ships empty with a "transcript coming soon" note in the UI. Building a real STT pipeline is a separate, larger piece of work than upload+playback; scoped out rather than blocking on it, same spirit as clustering/amendments/AI agents below.
- **Non-repudiation signing (§10) does not yet cover media arguments** — real signing over a file would need the client to hash the raw upload and sign that hash before it's transcoded (transcoding changes the bytes, so the signature can't be over the final stored file); a legitimate incremental feature, not built yet. Text arguments keep signing as before.
- **Cloudflare's own proxy in front of the app caps request bodies before they reach it** (100MB on Free/Pro plans) — independent of any limit VoteRight's own code sets. Confirm the zone's plan against real upload sizes if uploads start failing before the app ever sees them.
- **Security posture for running ffmpeg against untrusted uploads** (added 2026-08-15, prompted by the obvious question: an upload pipeline that feeds attacker-controlled bytes into a media parser is real attack surface, not a hypothetical one):
  - **The stored/served file is never the original upload.** Every upload is re-transcoded through ffmpeg to a known codec/container before it touches disk permanently or gets served to any viewer — this alone defeats the classic "disguise a payload as a media file, get it served back to other users" pattern (polyglot files, renamed executables, embedded scripts), since what comes out the other end is ffmpeg's own re-encoded output, not a copy of the attacker's bytes.
  - **`content-type` is always set from the DB's own `format` column, never sniffed from the uploaded file** — closes the "upload something that gets served as `text/html` and executes" class of attack outright; `X-Content-Type-Options: nosniff` on the serving route closes the browser-side reinterpretation angle too.
  - **On-disk filenames are always server-generated UUIDs, never derived from the client-sent filename** — no path traversal surface.
  - **ffmpeg/ffprobe child processes are killed on a wall-clock timeout (120s)** — a crafted file can make a demuxer hang or spin without ever exceeding the duration cap (which only reads container metadata, not real processing time); without a timeout that ties up a request indefinitely.
  - **Narrowed further 2026-08-15, still not fully closed**: ffmpeg/ffprobe still run as the same `voteright` OS user as the rest of the app — no seccomp, no container isolation, no separate low-privilege user, since real isolation needs root to set up (a new user, cgroups/namespaces) and `voteright` has none on this VPS. That's the deepest residual risk here, genuinely how most video-hosting pipelines work (accepting diverse real-world upload formats requires a broad decoder surface), not something a quick fix eliminates. What an unprivileged process CAN do to itself, self-serve, added this same day:
    - **`-protocol_whitelist file`** on every ffmpeg/ffprobe invocation — restricts the demuxer to plain local-file I/O, closing off a real ffmpeg CVE class where a crafted file's *content* (not just its name) references other locations (HLS playlists pointing at remote segments, the concat demuxer chaining in other local files) to reach internal network resources or read files the process has access to.
    - **`ulimit -v`/`ulimit -t` (~3GB virtual memory, 150 CPU-seconds)** wrapping the spawned process, via `exec` from a tiny shell wrapper so the existing wall-clock kill still targets the right PID. Bounds worst-case memory/CPU use from a malicious file — real, since this VPS also runs Safariis (DEPLOY.md), an unrelated app sharing the same RAM; a memory-exhaustion attack via crafted media could otherwise take both apps down, not just VoteRight.
    - **Not verified live**: the `ulimit` mechanism only runs on Linux (skipped entirely on Windows, where local dev happens) and there's no Linux environment available to test it in — confirmed with a real ffmpeg build that `-protocol_whitelist file` doesn't break normal transcoding, but the resource-cap values themselves (3GB/150s) are reasoned from typical ffmpeg memory footprints for this workload, not empirically measured against the real VPS. Watch the first few real video posts after deploy for unexpected transcode failures; the caps are single constants in `media.ts`, trivial to raise if too tight.
    - Real sandboxing (a dedicated low-priv user or container for the transcode step) is still the better fix if root ever becomes available for it — not built here.
  - **No per-user upload rate limit or disk quota yet** — nothing stops a verified user from posting many near-`MAX_UPLOAD_BYTES` files back to back and filling VPS disk. Same already-flagged gap as the general per-user argument rate limit above (`max_video_size_bytes`/rate-limit TBD), just more expensive per-post for video than text. Worth closing before this feature sees real traffic, not just testing.
...
### 9.2 Payment-as-verification (`payment_verified`, migration 085, 2026-08-19)

**Resolves §13 item 9 below with a different answer than that item anticipated.** That item framed the choice as address-only vs. government-ID document verification (Persona/Stripe Identity), and flagged the real equity-vs-Sybil tension in requiring an ID document. The owner's actual decision after that tradeoff was raised: reject document-based ID verification entirely — confirmed live neither vendor even has a usable free tier (Persona: no free plan, cheapest is $250/mo Essential; Stripe Identity: no free tier, $1.50/check) — and use **a successful payment itself** as the identity signal instead. A real card charge, ACH transfer, or reconciled mailed check, tied to a real name/bank account, stands in for a government ID check. No ID document upload (explicitly considered and dropped). Whether this is a *better* anti-Sybil signal than a document check is a real, open question — it sidesteps the "does everyone reliably possess ID" equity concern §13 item 9 raised, but introduces a different one (a payment floor is its own access barrier, plus it doesn't rule out one real person funding many identities the way a document check with liveness/uniqueness detection might).

**Scope**: only debate participation (proposing, seconding, arguing, calling the question) requires `payment_verified`. Referenda/mandate voting and accountability-campaign support are unchanged, still gated on `address_verified` alone (`app/src/lib/anon.ts`: `verifiedUserId()` unchanged; new, stricter `paymentVerifiedUserId()` used only by the debate-action routes). `payment_verified` requires having already reached `address_verified` first — payment is an additional identity-strength signal layered on top of already knowing a resident's jurisdiction, not a replacement for it.

**Multi-gateway from the start** (owner: "stripe is not the only payment gateway... include others like authorized.net") — `payment_settings` holds credentials for both Stripe and Authorize.Net at once, with an `active_gateway` selector, so switching doesn't lose the other's config. Both are tokenization-only on the client (Stripe Elements / Authorize.Net's Accept.js) — neither a raw card nor bank account number ever reaches VoteRight's own server. **The fee amount and both gateways' credentials are admin-configurable at runtime** (`/admin/payments`), the first such setting in this project — everything else is `.env.production`/VPS-deploy-gated. Check payment is a fully manual, gateway-free path: a generated reference code, admin reconciles by hand in the same screen.

**Per-admin identity for the `reconciled_by` field is now real** — see §10.3's admin-accounts entry below, built the same day after the owner asked for it mid-build. **Stripe verified end-to-end against a real test-mode account (2026-08-19, same day)** — PaymentIntent creation, confirming with a real test card, a validly-signed webhook, and `payment_verified` promotion all confirmed working via the real Stripe API. Caught and fixed a real bug in the process: the webhook handler checked the PaymentIntent's *allowed* payment method types rather than which one was actually used, so every successful payment (card or ACH alike) was being recorded as `'ach'` — fixed by retrieving the real `PaymentMethod.type`. Owner chose Stripe over Authorize.Net as the gateway to actually configure (same per-transaction rate, no $25/mo Authorize.Net base fee) — **Authorize.Net's integration remains completely untested against any live account**, and production itself has no keys configured yet (test-mode keys only, verified against local dev).

**Authorize.Net removed entirely (2026-09-05, migration 102, owner's explicit request)**, closing out the "untested against any live account" caveat above by removing the untested path rather than ever testing it: it never got configured or tested against a real account in the ~2.5 weeks since the paragraphs above were written, and the owner chose to fully commit to Stripe as the sole gateway instead of carrying it further as unused optionality. Removed: the `authorizenet.ts` gateway module, its own admin credentials card and the `active_gateway` gateway-choice dropdown in `/admin/payments` (with only one gateway left, there's nothing left to choose between, so the whole selector concept went with it, not just Authorize.Net's own fields), the Accept.js card-form path in `PaymentCheckout.tsx`, the dedicated `charge-authorizenet` route, the webhook dispatch branch, and mobile's corresponding "not available in the app yet" fallback state (now unreachable with Stripe as the only gateway). `payment_settings` dropped its `authorizenet_*` columns and `active_gateway` column; `payment_verifications.gateway` was kept (still meaningful per-transaction attribution) but its CHECK constraint narrowed to `'stripe'` only. The multi-gateway paragraphs above are left as-is as the historical record of the original 2026-08-19 decision, not as a description of the current state.

### 9.3 Demand-driven jurisdiction provisioning (migration 103, 2026-09-08)

**Owner's idea**: when a resident's address resolves to a real county VoteRight hasn't seeded local ballot detail for, tell them so plainly and start tracking demand — at 12 distinct residents, alert an admin to provision that county's local detail, and notify everyone who was waiting once it's done.

**The hook point isn't the rare "outside" resolution — it's the existing county-level fallback.** Since migration 059 (D6), every US state + D.C. + the 5 inhabited territories already has federal + state-level offices seeded, so a real US address essentially never resolves to true "outside" (no data at all) anymore — `jurisdictionForGeography` (`jurisdictions.ts`) already falls back to that state's bare jurisdiction row whenever the specific county isn't seeded, which is the actual "not yet covered" experience residents hit today. That fallback now also returns a `countyNotYetSeeded` flag carrying the Census state/county FIPS pair (and incorporated place name, if any) already computed for ballot routing — never the resident's raw address, which this codebase has never stored regardless.

**Privacy-preserving by construction, not by policy promise.** `debates.ts`'s `verifyAddress` records one demand signal per pseudonymous `user_id` per county — the exact same "one signal per verified user, count self-corrects" shape `accountability_campaign_supports` already established for a different feature, reused rather than reinvented. `jurisdiction_demand_signals` stores only the FIPS pair + place name (standard public geographic classification, the same category of data ballot routing already treats as non-personal) and the existing `user_id` — no new PII collection anywhere in the flow. Re-verifying the same address from the same identity can't inflate the count (a UNIQUE constraint on `(state_fips, county_fips, user_id)`).

**Two-stage confirmation before anyone is notified**, per the owner's own explicit design (asked directly, not assumed): crossing 12 signals fires an automatic admin alert (`adminNotify.ts`, no new alert channel), and the admin queue (`/admin/jurisdiction-demand`) auto-flags a county as "looks provisioned" once a real `jurisdictions` row + at least one office exists for that FIPS pair — but nothing notifies residents on that basis alone. An admin still has to look at the actual new data and click "Mark as provisioned" against VoteRight's own quality bar, matching the same "explicit admin action for a consequential state change" posture as check-payment reconciliation and mandate certification elsewhere in this project. That action notifies every distinct resident who signaled demand via the existing pseudonymous push/in-app notification system (`notifications.ts` — the same one CTQ/debate notifications already use, no email or contact info collected for this feature specifically) and deletes the tracking rows, per the owner's own spec ("remove that tracking from admin").

**User-facing**: shown once, at the moment of verification (`AddressForm.tsx`/mobile's `verify.tsx`), alongside the normal "address verified" confirmation — not a persistent ballot-page banner (that's `ballot_state_only_note`'s job, unchanged and still shown every time a state-only ballot is viewed for an unrelated reason: partial detail, not demand-tracking status).
...
- **Private signals with a retention rule** — referendum ballots (10.1), per-user `argument_agreement_votes`, `thread_reports` (member abuse reports) and `call_the_question_votes` (both live side by side as of migration 094, Section 12 — the second was briefly removed by 093 then restored, never "replaced" by the first), `push_tokens`/`notifications` (migration 094), and `jurisdiction_demand_signals` (migration 103, Section 12) — all deleted on a privacy request (`privacy.ts`'s `executeDeletion()`), same rule applied uniformly rather than re-derived per table. Agreement votes deserve the most care: they are the raw Pol.is matrix, a per-person ledger of agree/disagree stances across potentially hundreds of political statements — a *richer* political profile than any single referendum choice, gathered through a one-tap interaction no user experiences as going on the record. They are held only while genuinely needed (the open debate window, opinion-cluster recomputation, plus a short audit window) and then deleted. What persists: the denormalized per-argument tallies (`agree_count` / `disagree_count` / `pass_count`) and the final `opinion_clusters` snapshots, neither of which contains a user identity. Thread reports (the reporter's identity and stated reason) are similarly deleted on a privacy request; the resulting `forum_threads.closed_reason`, if an admin acted on one, persists as the public record instead. `jurisdiction_demand_signals` is a FIPS pair, not a raw address (Section 12) — still deleted on request, since it's a geographic-participation record tied to a user_id either way.
...
### 10.3 Application security hardening (added 2026-08-15)

A broader review (prompted directly: "what other vectors can be exploited...") turned up two confirmed gaps, both fixed the same day, plus findings worth recording even though nothing changed as a result:

- **Admin login brute-force protection.** `/api/admin/login` previously only delayed a failed attempt by 1 second, with a comment reasoning this was "serverless-friendly" since a per-IP store wouldn't survive across instances. That reasoning predates the move off Vercel (DEPLOY.md) — this app is now a single persistent systemd process on one VPS, so an in-memory store actually works. The old delay only throttled *sequential* attempts; concurrent requests each paid the same 1s delay in parallel, so enough concurrency could still work through a meaningful fraction of a TOTP code's 1,000,000-combination space within its ~90-second validity window. Replaced with a real per-IP lockout (`app/src/lib/loginRateLimit.ts`, unit-tested): 5 failures within 5 minutes locks that IP out for 15 minutes, keyed on `CF-Connecting-IP` (unspoofable through Cloudflare's Proxied mode) rather than `X-Forwarded-For` (which the rest of the app only ever uses for a coarse audit-context hash, where spoofing resistance doesn't matter as much). **Residual gap, not closable from app code**: if the VPS's origin IP is ever reachable directly, bypassing Cloudflare, `CF-Connecting-IP` is just whatever the direct caller sends and this lockout keys on nothing trustworthy — closing that needs an origin firewall rule (allow only Cloudflare's published ranges on the public port), which is infrastructure the app can't set up for itself.
- **Agreement votes and call-the-question votes were only gated client-side.** `debateDetail()` (10.2) already applies the right visibility rule when *listing* arguments — approved-only, open-thread-only — but `agreeVote()`/`ctqVote()` never re-checked either condition server-side; the debate page just didn't render the vote buttons otherwise. A direct POST to `/api/arguments/[id]/agree` or `/api/debates/[id]/ctq` could previously vote on a pending/removed argument or add votes to an already-closed thread, undermining the exact tallies (10.2) and CTQ outcome this section already treats as needing care. Both functions now check `moderation_status`/`forum_threads.status` before writing, inside the same transaction as the write itself. Verified against a real database (not just re-derived SQL) — a fixture covering an approved argument, a pending one, and an already-closed thread, exercised through the actual `agreeVote`/`ctqVote` functions. (`ctqVote()` and its route were themselves removed in migration 093, 2026-08-24 — Section 12 — but the same-transaction pattern this entry established lives on in `forceCloseThread()`.)
- **Per-admin accounts + per-screen permissions (migration 086, 2026-08-19)**, replacing the flat single-shared-TOTP model — asked for mid-build on the payments feature above, once it became clear "hand someone access to just the payments screen" had no way to work under one shared login. Each admin now enrolls their own TOTP secret (`admin_accounts.totp_secret`); the session cookie carries an HMAC-signed admin id, and permissions (`admin_screen_access`, one row per admin/screen grant) are read fresh from the DB on every request rather than cached in the cookie, so revoking a screen takes effect on that admin's *next* request, not just their next login. **Per-screen checkboxes, not named roles** — the owner's explicit choice over a smaller fixed set of roles, trading some setup friction per new admin for zero risk of a wrong grouping. A new account starts with **zero** screens granted, never inheriting the creator's own access. `/admin/admin-accounts` (itself a gated screen, `admin_accounts`) manages this — create an admin, see their TOTP enrollment secret exactly once, toggle per-screen checkboxes, disable/re-enable an account. `db/bootstrap-admin.mjs` seeds the first account from the existing `ADMIN_TOTP_SECRET` env value so the owner's already-enrolled authenticator entry keeps working with zero re-enrollment — **run this once after deploying migration 086, before the old shared-secret login stops being checked anywhere** (it already isn't; the rewritten `adminAuth.ts` has no code path left that reads `ADMIN_TOTP_SECRET` directly). Verified live against the real dev database: real TOTP login succeeds, a wrong code is rejected, a tampered session cookie is rejected, and a newly created admin granted only `payments` ends up with exactly that one screen and no others.
- **Recorded, not yet acted on**: this app's whole non-repudiation signing system (Section 10 elsewhere, `signing.ts`) is best-effort and unenforced — an automated client can simply omit a signature and still post, second, or propose, since `verifiedUserId()` is the only real gate. Verification itself is self-attested (address format + geocoder plausibility, no identity binding), and minting a fresh anonymous identity is free. Nothing here stops one person running `/verify` many times under different cookie identities to inflate seconds, agreement-vote tallies, or thread-report volume — a Sybil/ballot-stuffing risk that's structural to the trust model, not a bug a patch closes, and arguably more consequential for a civic platform than any single technical vulnerability. Deliberately not fixed as part of this pass: closing it means deciding what "verified enough" should mean for this platform, a product tradeoff, not a security patch to improvise.
- **Swept and found solid, worth recording so it isn't re-litigated**: every real admin API route enforces `isAdmin()` (grepped all 13 files under `api/admin/`; the 2 that don't are `login`/`logout`, which correctly don't need pre-auth); zero string-interpolated SQL anywhere in `src/` (fully parameterized throughout); the only `dangerouslySetInnerHTML` in the app is a hardcoded theme-boot script with no user input near it; referendum ballot casting (10.1) already checks `status = 'open'` and uses `SELECT ... FOR UPDATE` to prevent double-redemption races — the model `agreeVote`/`forceCloseThread` (Section 12) now follow; mobile session IDs are `randomUUID()`, not guessable; cookies are `SameSite=Lax` (`vr_uid`) / `SameSite=Strict` (`vr_admin_session`), real CSRF defense already in place.
- **Identity recovery via signing-key backup (migration 088, 2026-08-19)** — the flip side of the "minting a fresh anonymous identity is free" bullet above: losing one is *also* free, and until now, permanent. Owner asked directly whether a lost identity's thread (priorities, debate history, now subscription tier) could be restored without weakening anonymity — yes, using infrastructure that already existed but was never wired for it: `KeySettings.tsx`'s passphrase-encrypted signing-key backup/restore. `signing.ts`'s `ownerOfValidKey(fingerprint)` finds which `user_id` a key currently belongs to, using the exact same "most recent `user_key_events` row wins" rule `keyCurrentlyValid()` already relies on — which means a **revoked** key's most recent event is `'revoked'`, not `'registered'`, so a leaked-and-revoked backup file can never recover the old identity; that safety property came free from reusing the existing rule rather than inventing a new one. `anon.ts`'s `adoptIdentity(userId)` re-points the current session's cookie at the recovered identity by recovering its *original* anonymous cookie string (`users.auth_id` strips back to it) rather than minting a new one — so another device still holding the original cookie keeps working too, the same "signed in on multiple devices" property a real login system has, without ever needing a real login system. `clientSigning.ts`'s `importEncryptedBackup()` now calls this recovery-aware path (`/api/keys/recover`) instead of a blind re-register; `KeySettings.tsx` shows a distinct "welcome back" and reloads the page on an actual recovery, so every other piece of server-rendered state (priorities, debate history, subscription) picks up the restored identity immediately. No email, password, or real name anywhere in the mechanism — the recovery credential is the encrypted key file plus the passphrase, held only by the user, the same trust model a crypto wallet seed phrase uses. **Real tradeoff, stated plainly**: this raises the stakes of a leaked backup file from "someone can sign as you" to "someone can become you, including your paid subscription" — worth the passphrase warning already in the export flow carrying more weight now than when it was written. Verified live against the real dev database: a registered key correctly resolves to its owner, a revoked key correctly resolves to nothing, and the recovered cookie string correctly round-trips back to the exact same `user_id` on the next request.
...
| Unlocks | Export your own priorities as CSV; optional public "Supporter" badge on your posts | Everything in Supporter + a full match-report CSV export (every race on your ballot, every candidate's match band) | Everything in Patron + a personal API key for bulk, machine-readable access to the same sourced data already public on the site |
...
**Patron's match-report export is worth spelling out against the governing constraint**, since at first glance "bulk download of match results" sounds close to the line: it exports NO information a free user doesn't already have — every resident, any tier, already sees their own full match results for every race on `/matches`, unlimited, for free. Patron pays only for the *convenience of one bulk CSV download* instead of clicking through each race, the same principle Supporter's priorities export already relies on. Reuses the exact same district-narrowing (`ballotForJurisdiction`/`filterToOwnDistricts`) and scoring (`matchesForRace`) pipeline the free page uses — no parallel computation invented for the export.

### 14.2 Built vs. explicitly deferred (2026-08-19)

**Built**: the full billing spine (checkout, portal, webhook-driven tier sync), admin plan configuration, CSV export of a resident's own `voter_priorities` (Supporter+), a full match-report CSV export (Patron+), the Supporter badge, and a scoped v1 of bulk API access — one revocable key per Champion subscriber, one read-only JSON endpoint mirroring the already-public candidate/position/ballot data. **Verified live end-to-end 2026-08-19** — a real live $5 Supporter charge, refunded afterward. Two real bugs surfaced only by this live run and fixed the same day: the subscriptions webhook destination had never actually been created on Stripe's side (the charge succeeded on Stripe but VoteRight never heard about it until the destination existed), and the checkout/portal routes built their Stripe return URLs from `request.url`'s origin directly, reproducing the exact "redirects to localhost:3001" bug already fixed once on 2026-08-14 — corrected via a new shared `siteOrigin()` helper in `redirect.ts`, same `SITE_URL`-first fix, reused by both routes. Match-report export is also verified live against the real dev database (a real Montgomery County District-3 resident's full 26-race ballot, correctly district-narrowed, real candidate names/parties). **KYC does not apply to VoteRight's own subscribers** (researched, not assumed — it's Stripe's obligation toward the account holder, only extending to end customers under a Connect/marketplace model, which this isn't); transaction history and receipts are handled by Stripe's own "Successful payments"/"Refunds" customer-email toggles plus the Billing Portal already wired in — no accounts system needed. See `docs/BACKLOG.md` for the full resolution.
